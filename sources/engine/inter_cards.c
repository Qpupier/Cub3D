...
/*   Updated: 2021/04/23 10:22:36 by qpupier          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static inline int	in_map(t_map *map, int x, int y)
{
...
float	intersec_planes_e(t_param *p, t_line line, float *r_w, float *r_h)
{
	...
	while (++i <= p->map->w)
	{
		if (p->map->p_e[i] && inter_line_plane_x(line, -i, &result, &t) \
				&& t > 0 && result.z >= 0 && result.z < 1 && result.y >= 0)
		{
			x = i;
			y = result.y;
			if (in_map(p->map, x, y))
...
float	intersec_planes_s(t_param *p, t_line line, float *r_w, float *r_h)
{
	...
	while (++i <= p->map->h)
	{
		if (p->map->p_s[i] && inter_line_plane_y(line, -i, &result, &t) \
				&& t > 0 && result.z >= 0 && result.z < 1 && result.x >= 0)
		{
			x = result.x;
			y = i;
			if (in_map(p->map, x, y))
